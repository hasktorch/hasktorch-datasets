...
def main(args=None) -> None:
    ...
    if args.mode == "trace":
        ...
        decoder_inputs = tokenizer(args.decoder_input, return_tensors="pt")  # Batch size 1
        traced_script_module = torch.jit.trace(model, (inputs.input_ids, inputs.attention_mask, decoder_inputs.input_ids, decoder_inputs.attention_mask))
        traced_script_module.save(args.output)
    elif args.mode == "state-dict":
        model = T5ForConditionalGeneration.from_pretrained(args.model, torchscript=False)
